...
const char *in6ToChar (struct in6_addr *addr) {
	...
	snprintf(s_address, sizeof(s_address), "%x%x%x%x", ntohl(base[0]), ntohl(base[1]), ntohl(base[2]), ntohl(base[3]));
	return s_address;
} // in6ToChar
...
int main(int argc, char *argv[]) {
	...
	const char *ntop_result = inet_ntop(AF_INET, &inet_addr, inet_name, sizeof(inet_name));
	...
	strncpy(inet6_name, "FE80::0230:65FF:FE06:6523", INET6_ADDRSTRLEN);
	...
	// Convert from numeric to presenation, inet6_addr -> inet6_name
	ntop_result = inet_ntop(AF_INET6, &inet6_addr, inet6_name, sizeof(inet6_name));
